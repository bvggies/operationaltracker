...
router.post('/login', async (req, res) => {
  try {
    ...
    // Find user - handle both password_hash and password columns
    ...
    // Check password - handle both password_hash and password columns
...
router.get('/me', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT id, username, email, full_name, first_name, last_name, role, is_active, created_at FROM users WHERE id = $1',
      [req.user.id]
    );
    ...
    const user = result.rows[0];
    // Handle both full_name and first_name/last_name
